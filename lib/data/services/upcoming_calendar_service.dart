...
class UpcomingCalendarService {
  ...
  Future<List<AggregatedItem>> loadMergedCalendar({
    ...
    try {
      ...
      BetterPostersService.registerAll(mergedItems);
  ...
  List<AggregatedItem> _filterAndFormatRadarrItems(List<AggregatedItem> rawItems) {
    ...
    for (final item in rawItems) {
      ...
      String? subtitleText;
      if (showDate) {
        final dateStr = _relativeDayLabel(targetReleaseDate, now) ??
            _formatDateHuman(targetReleaseDate);
  ...
  List<AggregatedItem> _formatSonarrItems(List<AggregatedItem> rawItems) {
    ...
    return rawItems.map((item) {
      ...
      if (showDate && showEpisodeInfo) {
        final dateStr =
            _relativeDayLabel(airDateUtc, now) ?? _formatDateHuman(airDateUtc);
  ...
  /// "Today" / "1 Day" / "N Days" for release dates within the coming week,
  /// otherwise null (callers then show the absolute date). Compared by
  /// calendar day so an episode airing later today reads "Today". Dates
  /// outside the 0–7 day window, including past dates, keep absolute dates.
  static String? _relativeDayLabel(DateTime target, DateTime now) {
